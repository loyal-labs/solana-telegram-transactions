...
type CommunityActiveUsersBarChartProps = {
  data: Array<{
    date: string;
    activeUsers: number;
  }>;
...
export function CommunityActiveUsersBarChart({ data, stats }: CommunityActiveUsersBarChartProps) {
    ...
    <Card className="h-full w-full py-4 sm:py-0">
      <CardHeader className="flex flex-col items-stretch border-b !p-0 sm:h-48 sm:flex-row">
        <div className="flex flex-1 flex-col justify-center gap-1 px-6 py-4 sm:py-6">
          <CardTitle className="font-bold">Active users per day</CardTitle>
          <CardDescription>Daily unique message senders in the last 30 days</CardDescription>
        </div>
        <div className="flex">
      ...
      <CardContent className="px-2 sm:p-6">
        <ChartContainer config={chartConfig} className="aspect-auto h-[250px] w-full min-w-0">
          ...
          >
            ...
            <ChartTooltip
              content={
                <ChartTooltipContent className="w-[150px]" labelFormatter={formatTooltipDate} />
              }
            />
