...
export type SwapSheetProps = {
  ...
  // Result state props
  view?: "main" | "selectFrom" | "selectTo" | "result";
  onViewChange?: (view: "main" | "selectFrom" | "selectTo" | "result") => void;
...
export default function SwapSheet({
  ...
  // State
  const [activeTab] = useState<"swap" | "secure">("swap");
  const [internalView, setInternalView] = useState<"main" | "selectFrom" | "selectTo" | "result">("main");

  // Use controlled view if provided, otherwise internal
  const view = viewProp ?? internalView;
  const setView = useCallback(
    (newView: "main" | "selectFrom" | "selectTo" | "result") => {
      if (onViewChange) {
  ...
  useEffect(() => {
    if (open && view === "main") {
      onValidationChange?.(isSwapValid);
    } else {
      onValidationChange?.(false);
    }
  }, [open, view, isSwapValid, onValidationChange]);
