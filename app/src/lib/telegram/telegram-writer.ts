...
/**
 * Sends a draft with fallback to regular message + edit pattern.
 * This provides streaming-like experience even if sendMessageDraft fails.
 *
 * @param bot - Grammy bot instance
 * @param params - Draft parameters
 * @param existingMessageId - If provided, edits this message instead of sending draft
 * @returns Object with success status and optional message ID
 */
export async function sendDraftWithFallback(
  bot: Bot,
  params: SendDraftParams,
  existingMessageId?: number
): Promise<{ success: boolean; messageId?: number }> {
  // If we have an existing message, edit it
  if (existingMessageId) {
    try {
      await editMessage(bot, params.chatId, existingMessageId, params.text, params.parseMode);
  ...
  try {
    const success = await sendDraft(bot, params);
    if (success) {
      return { success: true };
    }
  } catch (error) {
    console.warn("sendMessageDraft failed, falling back to sendMessage + edit:", error);
  }

  // Fallback: send initial message that we'll edit
  try {
    const messageId = await sendMessage(bot, {
    ...
    console.error("Fallback sendMessage also failed:", error);
    return { success: false };
  }
}
